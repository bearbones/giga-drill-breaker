...
`--source` is optional: with none, every entry of the compilation
database is indexed (see [File selection](#file-selection) for
`--source-re` and `--source-list`). Progress goes to stderr; when the bake finishes the index is written to
...
With no `--source` at all, `megascope index` takes every entry of the
compilation database. Narrow it with:
...
whatever the base set is. `megascope index` reports how many TUs each
filter dropped on stderr and refuses to bake an empty selection.
